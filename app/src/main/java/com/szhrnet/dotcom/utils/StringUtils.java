...
public enum StringUtils {
   ...
   * @return Hashtable 对象
   */
  @SuppressWarnings({ "rawtypes", "unchecked" }) private static Hashtable GetAreaCode() {
  ...
  public static String insertString(String beforStr, String insertStr, int position) {
    StringBuffer stringBuffer = new StringBuffer(beforStr);
    return stringBuffer.insert(position, insertStr).toString();
  }

  /** 此方法仅提供 香港六合彩 更换名称 */
  public static String changeLHCNameText(String string) {
    if (TextUtils.isEmpty(string)) return string;
    if (string.contains("六合彩")) return string.replace("六", "⑥");
    return string;
  }
