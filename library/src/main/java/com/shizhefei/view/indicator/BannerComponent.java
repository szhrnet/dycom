...
public class BannerComponent extends IndicatorViewPager {
  ...
  private void initViewPagerScroll() {
    try {
      ...
      scroller = new DurationScroller(viewPager.getContext());
  ...
  @Override protected void iniOnItemSelectedListener() {
    indicatorView.setOnItemSelectListener(new Indicator.OnItemSelectedListener() {

      @Override public void onItemSelected(View selectItemView, int select, int preSelect) {
  ...
  @Override protected void iniOnPageChangeListener() {
    viewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
      @Override public void onPageSelected(int position) {
        ...
        if (onIndicatorPageChangeListener != null) {
          onIndicatorPageChangeListener.onIndicatorPageChange(indicatorView.getPreSelectItem(),
              mAdapter.getRealPosition(position));
        }
      }

      @Override
      public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        indicatorView.onPageScrolled(mAdapter.getRealPosition(position), positionOffset,
  ...
  @Override public void setCurrentItem(int item, boolean anim) {
    ...
    if (count > 0) {
      ...
      if (Math.abs(add) > viewPager.getOffscreenPageLimit()
          && viewPager.getOffscreenPageLimit() != count) {
  ...
  @Override public void setAdapter(IndicatorPagerAdapter adapter) {
    if (!(adapter instanceof LoopAdapter)) {
      throw new RuntimeException(
          "请设置继承于IndicatorViewPagerAdapter或者IndicatorViewPagerAdapter的adapter");
    }
