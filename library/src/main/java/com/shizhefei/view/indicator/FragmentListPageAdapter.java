...
public abstract class FragmentListPageAdapter extends PagerAdapter {
  ...
  @Override public Object instantiateItem(ViewGroup container, int position) {
    ...
    String tag_index = arguments.getString("TAG_INDEX");

    mCurTransaction.add(container.getId(), fragment, tag_index);

    return fragment;
  }

  /** 根据tag 查找对应Fragment index。 */
  public int findFragmentByTag(String tag) {
    int index = -1;
    Fragment fragmentByTag = mFragmentManager.findFragmentByTag(tag);
    if (fragmentByTag != null) {
      index = mFragments.indexOfValue(fragmentByTag);
    }
    return index;
  }
