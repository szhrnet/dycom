...
public abstract class TableDataAdapter<T> extends ArrayAdapter<T> {
   ...
   * @param data The data which shall be displayed in the table.
   */
  protected TableDataAdapter(final Context context, final TableColumnModel columnModel,
      final List<T> data) {
  ...
  /**
   * Gives the {@link Context} of this adapter. (Hint: use this method in the {@code
  ...
  public final View getView(final int rowIndex, final View convertView, final ViewGroup parent) {
    ...
    final AbsListView.LayoutParams rowLayoutParams =
    ...
    try {
      ...
      Log.w(LOG_TAG, "No row date available for row with index "
          + rowIndex
          + ". "
          + "Caught Exception: "
          + e.getMessage());
    ...
    for (int columnIndex = 0; columnIndex < columnModel.getColumnCount(); columnIndex++) {
      ...
      final LayoutParams cellLayoutParams = new LayoutParams(cellWidth, LayoutParams.MATCH_PARENT);
  ...
  /**
   * Sets the {@link TableDataRowBackgroundProvider} that will be used to define the table data rows
   * background.
   *
   * @param rowBackgroundProvider The {@link TableDataRowBackgroundProvider} that shall be used.
   */
  protected void setRowBackgroundProvider(
      final TableDataRowBackgroundProvider<? super T> rowBackgroundProvider) {
